...
import { buildSalesEmailHtml, buildCustomerEmailHtml, type OrderEmailData } from '../helpers/email-templates';
...
module.exports = factories.createCoreController("api::order.order", ({ strapi }) => ({
    ...
    async confirmStripeSession(ctx) {
        try {
            ...
            const lineItemsResponse = await stripe.checkout.sessions.listLineItems(session_id);
            ...
            setImmediate(async () => {
                ...
                try {
                    ...
                    console.error("❌ Error enviando email a ventas:", emailError.message);
                }
